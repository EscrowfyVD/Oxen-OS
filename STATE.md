...
>
> **Last updated** : 2026-07-16 (Phase 3 **100% in prod** ; post-Phase-3 closeout done ; Trigify reactive layer (PR #12) + recompute-cron dedicated config (PR #13) merged — Railway cron **service** live on `47 * * * *` (runtime `DATABASE_URL` = prod still to confirm on first run). **In flight : AIRA F2 (Pre-Meeting Briefings, LemCal) — PR0(#15)+PR1(#14)+PR2(#16) merged (webhook `/api/webhooks/lemcal` live in prod, INERT until LemCal env+URL set) ; PR3a(#17)+PR3b(#18) merged (refresh cron — Railway `*/15` service to create). **Apollo enrichment (replaces Clay) — PR-W(#19)+PR-Y(#20)+PR-Z(#21) merged (full engine live: enum/columns + `apollo.ts` client + mappers + batch runner + cron) ; PR-Xa(#22, Clay-webhook removal + helper rename) in review. NEW: Apify scraped-signals → CRM/scoring — PR1(#23)+PR2(#24)+PR2.5(#25)+PR3a-migration(#26) merged (`ProcessedSignal` dedup ledger **live in prod**) ; PR3a-wiring(#27) merged ; PR3b-seed(#28) merged **+ seed RUN in prod (apify_f/apify_g confirmed in registry)** ; PR3b-pipeline(#29) merged ; **cycle-1 ran** (33 items stored, 0 routed — Crunchbase Title-Case field-bug + 9 net-new Job Board ICP prospects, all legit-no-match) ; **PR3c-a(#30) merged** (no-match capture live — first CRM-writing slice ; post-merge baseline clean) ; PR3c-b-migration(#31) merged (**columns live in prod** — information_schema + migrate-deploy ledger clean) ; PR3c-b-score(#32) merged (company score live — writer sans lecteur, inert until the sweep) ; Crunchbase hotfix(#33) merged (Title-Case mapping + 90d recency + stable dedup key) ; funnel instrumentation(#34) merged (per-step counters in Job.result) ; PR3c-b-enrich slice 1/4(#35) merged (attempt-tracking columns + sweep index live in prod) ; slice 2/4(#36) merged (config + T re-wire, adversarial-reviewed clean) ; **slice 3/4(#37) merged** (Apollo `searchOrganizations` + `searchPeople`) ; **v3 ScoringConfig SEEDED in prod** (enrichment block active — Andy can tune T/caps/titles ; v1/v2 preserved inactive) ; **Apollo cron health check** : service `appollo-cron` `DATABASE_URL` was mispointed to a dead `localhost:5432/oxen_os_dev` → **CRASHED** each tick (Prisma init error at pass-1), zero prod writes → **Vernon FIXED the env** to the prod DB ; **Apollo cron exit fix (#38) MERGED** : all 3 cron entrypoints (`run-apollo-enrichment`/`run-apify-ingestion`/`run-recompute`) disconnect the **shared** `@/lib/prisma` singleton + exit explicitly via try/catch/finally (was a throwaway-client disconnect + no exit → Active-freeze on the first SUCCESSFUL run) ; **both fixes VALIDATED on prod** — the 07:00Z tick ran on the fixed build (`ba05f15`), connected to prod, clean no-op (backlog 0), terminal SUCCESS (no freeze) ; **Apify cron verified NOT frozen** (ticks every 10 min & exits each empty run — logs 05:50/06:00/06:10 ; 2 cycle-1 Jobs completed, 0 pending) → the "no captures in prod" finding = **actors idle / no new Jobs since 07-07**, NOT a frozen cron (caveat : every healthy tick observed is an empty `jobs:0` run ; a heavy successful run's exit stays unproven — the fix is the guarantee for the batch that matters) ; **slice 4/4 (pass-3 enrichment sweep, DRY-RUN by default) done local** on `apify-pr3c-b-enrich-sweep` — Gate 1 on index #35 + derived monthly cap + failure marker + credit instrumentation ; `enrichPerson` opt-in reveal + `upsertPersonFromApollo` create-or-link path + optional `enrichment.dryRun` (seeded true) ; **adversarially reviewed (4 lenses)** — dry-run zero-spend/zero-write verified SAFE, live-path findings fixed (case-insensitive contact lookup, no cross-company hijack/null, best-effort domain-claim + contact-upsert, credit counted at reveal) ; ships NO-SPEND until Vernon flips `enrichment.dryRun=false` after a dry-run report on real captures. **The Apify→CRM enrichment chain is now CODE-COMPLETE** (activation gated on Andy's actors producing captures + the dryRun flip) ; **1st REAL cycle (2026-07-10)** : 4 webhooks → 4 Jobs completed ; crunchbase 5/7 keyword-survive (Title-Case hotfix confirmed) ; jobboard **4/20** (16 "Head of Finance" died at keyword — the expected bottleneck) ; **9 net-new companies, 0 dup**, all single-signal (max score 6 < T) → **0 Gate-1 candidates**, dry-run sweep would-spend **0** (validated in the prod cron log) ; **`Company.acquisitionSource`** capture-source marker (pipeline-UI source filter's missing piece) — PR0 migration `20260710120000` (deploy-ahead, byte-identical to `prisma migrate diff`) + PR3c-a wiring (declared per-actor in ROUTABLE_ACTORS, CREATE-only, never overwrites a matched/attached company) + `backfill-company-acquisition-source.ts` for the 9 existing (**dry-count 9** = 5 crunchbase + 4 jobboard ; Vernon runs `--apply` after merge) ; front-end filter = later slice.** ; **🔴 LLM MODEL MIGRATION (2026-07-14, LIVE prod incident)** : `claude-sonnet-4-20250514` was RETIRED 2026-06-15 → every AI `messages.create` (all 32 call sites) 404'd `not_found_error` for a MONTH with no fallback ; unnoticed because **10/32 sites swallow the error** and still return HTTP 200 / complete their job (news-relevance scorers silently score every item 0 → "irrelevant" ; intel cron + bulk CRM loops push errors into unread body arrays). Confirmed via prod 404 logs (`check-upcoming` brief-gen) + account models-list (retired ID absent, `claude-sonnet-4-6` present). FIX (branch `fix-retired-model-migration`, **local — Mode-B STOP**) : centralized as `CLAUDE_MODEL` — one constant per deployable package (`src/lib/ai/model.ts` app + `workers/ai-worker/src/model.ts`, separate pkg / no `@/` alias) ; migrated all **32 hardcoded literals → `claude-sonnet-4-6`** ; fs guard-test asserts no stray literal + retired string gone (SDK-mocked unit tests can't catch this class of bug — the guard is the compensating control) ; **ZERO** temperature/top_p/top_k/thinking anywhere → the 4.6 breaking-param changes don't apply ; CLAUDE.md "never hardcode, import `CLAUDE_MODEL`" rule added. AC green (app tsc 0 / eslint +0 new / vitest 936 / next build 0 ; ai-worker tsc = 4 **pre-existing** non-CI-gated errors, unrelated). **Deferred hardening** (out of scope for the restore) : the 10 error-swallowing sites + 16 high-parser-fragility parsers + no cron/worker alerting.** [→ **MERGED #43**, deployed `f2632f1` on all 4 services (app/AI-Worker/2 crons, auto-deploy), 404 gone from live logs — incident closed] ; **🟠 LLM ERROR-HANDLING HARDENING (2026-07-15, Phase-2 PREREQUISITE, branch `llm-error-hardening`, local — Mode-B STOP)** : addresses the post-mortem's real danger — 10/32 sites SWALLOW an LLM error into a plausible NEGATIVE (harmless today only because the target tables are dormant/empty ; Phase-2 Reddit/News activates exactly those paths). RULE enforced : an LLM error must PROPAGATE (fail job / 5xx) or be an explicit not-evaluated state — never a fabricated value (score 0 on error is a lie). (1) **News scorers** (worker `index.ts:481` + `seo/news/auto-scan` + `seo/news/scan`) now use a shared `src/lib/ai/score-news-article.ts` whose API call is UNWRAPPED → an Anthropic error PROPAGATES (never a fake score:0) ; the worker job FAILS visibly+retryable (2nd `catch{continue}` swallow layer rethrows APIError), the routes return 503 ; **NO migration** — chose fail-the-job over error-state rows (a genuine irrelevant stays a row ; an LLM error writes NOTHING). The other 5 swallowers (`check-followups`/`health-check-all`/`score-all`/`intel-cron`/`check-upcoming`) already don't fabricate — each now ALERTS on an APIError. (2) **Alerting** : `src/lib/ai/llm-alert.ts` `notifyLlmFailure` reuses the BD Telegram broadcast (`CRM_BD_EMAILS`+`notifyEmployee`), throttled first-failure-then-quiet 6h, never-throws ; worker variant (`workers/ai-worker/src/lib/llm-alert.ts`) via Sentry + raw Telegram (separate pkg). (3) **Canary** : `POST /api/cron/llm-canary` (CRON_SECRET) + worker boot canary — a real 1-token call to the ACTUAL `CLAUDE_MODEL` (deliberately NOT a cheaper proxy — must detect THIS model's retirement) → alert+503 on failure. **Parsers (16 fragile) OUT of scope** (parse-fail still returns score 0 — noted). Tests (13 new) : LLM error → no fake row + 503 + alert ; genuine low score → real row ; alert fires/throttles/never-throws ; canary ok/error. AC green (app tsc 0 / eslint +0 / vitest **949** (+13) / build 0 ; ai-worker tsc = same 4 pre-existing non-CI-gated). **OPS after merge** : point Railway Cron at `/api/cron/llm-canary` (~hourly) so the next deprecation is loud.** [→ **canary trigger WIRED** : `.github/workflows/llm-canary.yml` (GHA `23 * * * *` curls `/api/cron/llm-canary` w/ `CRON_SECRET` — verified SET in prod + GH secrets ; the route is HTTP-only, no script — repo route-crons are GHA-driven, not Railway ; mirrors `signal-decay.yml` ; worker boot-canary already fires per-deploy). **OPS loop closed.**] ; **🟡 LLM PARSE-HARDENING PHASE 0 (2026-07-15, branch `llm-parse-hardening-phase0`, local — Mode-B STOP)** : the output-side of the false-result class (call succeeds, OUTPUT unusable → fabricated business result). Recon found 17 fragile parsers ; Phase 0 = the **5 FAKE-DEFAULTERS ONLY** (surgical bleed-stop, NOT the shared parser=Phase 2, NOT structured output=Phase 3). RULE (= #44, one layer down) : unusable output (parse-fail OR valid-but-incomplete) must NEVER become a fabricated value. Fixes : (1) `marketing/compliance-check` — the catch-fabricate `medium/50/[]` (a persisted fake regulated audit) → now persists `status:'error'` (null risk/score, clearly NOT a verdict) + alert + 502 ; **NO migration** ('error' is a free String status). (2/3/4) `score-all` + `score-lead` + `ai-worker handleScoreLead` — the `total ?? Σ(f ?? 0)` chain fabricated `icpScore:0/tier_3` + bumped `lastScoredAt` → now a missing/non-numeric total **throws** (no write, `lastScoredAt` NOT bumped → re-scored next run, fail 500/failJob + alert) ; `score-lead` had NO alert → added. (5) `ai-worker scoreArticle` + its app twin `src/lib/ai/score-news-article` — no-JSON/missing-score fabricated `{score:0,'irrelevant'}` → now **throws** `LlmOutputError` (scan job fails / route 503 + alert). **Alerting widen (the #44 gap)** : `notifyLlmFailure` was gated on `instanceof Anthropic.APIError` so a parse `SyntaxError` was invisible → new `isLlmFailure()` (APIError | SyntaxError | LlmOutputError) replaces that gate at every call site — an OUTPUT failure now also pings the BD Telegram alert. Tests (12 new) ; AC green (app tsc 0 / eslint +0 / vitest **961** (+12) / build 0 ; ai-worker tsc = same 4 pre-existing). **Deferred** : Phase 2 (centralize the 5×-duplicated `parseJsonFromText` into one robust shared parser + truncation detection + raise low `max_tokens`) ; Phase 3 (structured-output/tool-use for the fixed-schema sites — the REAL fix that erases parse + fabrication together) ; the Phase-2 classifier MUST be born on structured output (else an 18th fragile parser).] ; **🟢 LLM PARSE-HARDENING PHASE 2 (2026-07-15, branch `llm-parse-hardening-phase2`, local — Mode-B STOP)** : the robustness/centralization layer (behavior guarantee already shipped Phase 0/1). **ONE shared parser** `src/lib/ai/parse-llm-json.ts` (+ `workers/ai-worker/src/lib/parse-llm-json.ts` mirror, separate pkg) — `parseLlmJson<T>(response)` : (1) **truncation FIRST** (`stop_reason==='max_tokens'` → `LlmTruncationError`, a new `LlmOutputError` subclass so the alert says "raise max_tokens") ; (2) text from ALL text blocks via type-filter (not the fragile `content[0]`) ; (3) **string-aware balanced-brace** extraction of the outermost `{..}`/`[..]` (ignores fences+prose ; unclosed = truncation) ; (4) throws typed, **never fabricates**. **Adopted at all 18 call-sites (16 files)** — the 5 Phase-0 fabricators (guards kept), the 9 crash-visible (still 500/failJob), the 3 silent-drop ; **removed all 5 duplicated `parseJsonFromText`** ; `auto-insights` (was a silent 200+empty array) now throws→surfaces+alerts. **Truncation root-cause fix — raised 7 too-low `max_tokens`** : classify-lead 500→1024, score-all 512→1024, health-check-all 512→1024, research 2048→4096, generate-meeting-brief 2048→4096, auto-insights 2048→4096, keywords/discover 1024→4096 (worst — ~20-object array at 1024) ; left the 8000 (HTML) + adequate ones (cost). ⚠️ **process note** : a Workflow of agents editing the tree CLOBBERED my uncommitted edits (git-reset to launch snapshot) — recovered by re-applying all 16 sites by hand ; lesson : never run a file-editing Workflow over uncommitted work. Tests (13 new : balanced extraction, fence/prose, content-filter, truncation via stop_reason+unclosed, typed throws). AC green (app tsc 0 / eslint +0 / vitest **973** (+12) / build 0 ; ai-worker tsc = same 4 pre-existing ; **NO migration**). **Deferred** : Phase 3 (structured-output/tool-use for the fixed-schema sites — erases parse+fabrication together ; the Phase-2 classifier born on it).]** ; **⚪ HIDE UNUSED MODULES — tasks + wiki (2026-07-16, branch `hide-tasks-wiki-modules`, local — Mode-B STOP)** : both modules unused (no data to preserve, Vernon confirmed) → hidden from the ENTIRE UI + Telegram surface, **reversibly** via one shared flag module `src/lib/hidden-modules.ts` (`TASKS_HIDDEN`/`WIKI_HIDDEN`, both typed `boolean` so the gates aren't const-folded / dead-code-eliminated ; flip to `false` + rebuild to fully restore). **NO deletion, NO migration, NO data touched.** Web : nav filter (Sidebar) + command-palette filter + dashboard quick-actions filter + home quick-action dispatch guard + all 5 page routes `redirect("/")` at top (tasks + wiki + wiki/new + wiki/[slug] + wiki/[slug]/edit) + the conferences "View in Wiki" link gated (auto-gen conference report pages become unviewable — accepted ; conferences STILL create wiki pages in the background, untouched). Telegram (the "everywhere" part) : `/tasks` command + `cmd_tasks` callback disabled, all `/tasks` adverts dropped (2 help lists + linked-account msg in webhook & register + the short-message tip) ; the meeting-note handler's `prisma.task.create` loop gated (no more silent task rows) ; the weekly-digest's 6 task queries → `0`/`[]` + the "Tasks completed" line gated. Left AS-IS (harmless background reads/writes) : conference report generator (`wikiPage.create`), `wiki/ask` (degrades gracefully, no pages), the passive task-stat readers (dashboard count / Sentinel chat / digest — read 0, never error). AC green (app tsc 0 / eslint **+0 new** — identical 17 problems (5 err) on the touched set before+after, all pre-existing `set-state-in-effect` on untouched lines / vitest **973/973** / next build 0) ; browser-verify N/A (app auth-gated + `next dev` binds prod `DATABASE_URL`) → proven by next build + a full static entry-point audit instead. Reversible : delete the branch or set both flags `false`.]**)

---

## TL;DR

- **Repo health** : `npm run build` green ; tests **932/932** passing (94 files,
...
- **Apify scraped-signals → CRM / scoring** (Andy PRD) — rides existing infra, NOT
  ...
  **Crunchbase hotfix done** (branch `apify-crunchbase-hotfix`, local) — resolves the
  ...
  (Hotfix merged **#33**.) **Funnel instrumentation merged (#34)** —
  ...
  **PR3c-b-enrich — recon done (2026-07-09) + slice 1/4 done local** (branch
  ...
  (promis depuis #31) : `ON ("intentScore" DESC) WHERE enrichedAt IS NULL AND domain IS
  NULL AND intentScore IS NOT NULL` — DDL dans la migration SQL uniquement (Prisma PSL
  ...
  Sentinel. **Slice 2/4 merged (#36)** : bloc
  `enrichment` sur ScoringConfig — `.strict()` interne, clé top-level `.optional()`
  (deploy→seed safety : v2 sans clé valide toujours → pas de throw du loader) ;
  buildScoringConfigV3 + seedScoringConfigV3 (v3 active) ; **T re-wire** (chemin live) :
  `config.enrichment?.gate1Threshold ?? COMPANY_ENRICH_THRESHOLD` (défaut 10 =
  byte-identical) + résultat expose `threshold` ; revue adversariale 4-lentilles =
  clean ; +15 tests. **Slice 3/4 done local** (branch `apify-pr3c-b-enrich-client`) :
  `searchOrganizations({name, linkedinUrl?})` (mixed_companies/search → org+domain ;
  disambig linkedin match sinon nom ≥0.85) + `searchPeople({organizationId, titles[],
  seniorities[]})` (mixed_people/search → people SANS email ; empty=[] vs error=null ;
  level-agnostic, slice 4 l'appelle 2× DM+ops) — mêmes gardes (skip-no-key/never-throw/
  429) ; types `ApolloOrgSearchResult`/`ApolloPersonSearchResult` exportés ; +12 tests
  (mockés, zéro appel réel, zéro crédit). apollo.ts existant intouché. **Slice 4 next** :
  pass-3 sweep (Gate 1 config, intentScore DESC,
  budget dérivé count(enrichedAt>=month-start) — exact tant que pass-1/2 inertes,
  raffiner à leur allumage ; 2 contacts create → `recomputeCompanyContacts` = ICP
  in-pass → Gate 2 lisible). Dettes actées : cross-source event-dedup (réveil à la
  2e source même event-type) ; crédits plan-dépendants ; phone = slice réservée.
